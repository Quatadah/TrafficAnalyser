...
public class TraficByDateHourTypeAndSensor {
    public static  class TraficByDateHourTypeAndSensorMapper  extends Mapper<LongWritable, Text, Text, Text> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            try {
                ...
                if (commonData != null ){
                    type.set(commonData.getDate()+","+commonData.getTime().split(":")[0]+","+commonData.getVehicleType().toUpperCase(Locale.ROOT)
                                    +","+commonData.getSensorType());
    ...
    public static class TraficByDateHourTypeAndSensorReducer extends Reducer<Text, Text, Text, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            context.write(new  Text(key.toString().split(",")[0]+", "+key.toString().split(",")[1]),
                    new Text(count+","+key.toString().split(",")[2]+","+key.toString().split(",")[3]));
        }
    }
}
